---
name: prototype
description: Build throwaway logic or UI prototypes that answer a specific design question quickly, then capture the answer and delete or absorb the code. Use when uncertainty is about state behavior, API shape, or visual direction rather than production implementation.
---
...
4. **Skip production polish.** No broad test suite, no production-grade error handling, no reusable abstraction. Do handle startup failures and invalid demo inputs clearly enough that the prototype remains runnable.
5. **Surface the state.** After every action (logic) or on every variant switch (UI), print or render the full relevant state so the user can see what changed.
6. **Delete or absorb when done.** When the prototype has answered its question, either delete it or fold the validated decision into the real code — don't leave it rotting in the repo.

## Run contract

- Provide one exact command and the working directory it expects.
- Normal user exit should return `0`; startup/runtime failure should return non-zero with a short cause.
- If the prototype needs env vars or fixtures, provide safe defaults or print the missing requirement before exiting.
- Keep real mutations disabled unless the prototype's explicit question is about the mutation flow.
